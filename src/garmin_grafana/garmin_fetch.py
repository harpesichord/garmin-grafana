...
INFLUXDB_HOST = os.getenv("INFLUXDB_HOST",'your.influxdb.hostname') # Required for v1 and v3
INFLUXDB_PORT = int(os.getenv("INFLUXDB_PORT", 8086)) # Required for v1 and v3
...
INFLUXDB_DATABASE = os.getenv("INFLUXDB_DATABASE", 'GarminStats') # Required for v1 and v3 (database name)
INFLUXDB_V2_URL = os.getenv("INFLUXDB_V2_URL", '') # InfluxDB V2 URL, required only for InfluxDB V2 (e.g., http://localhost:8086)
INFLUXDB_V2_TOKEN = os.getenv("INFLUXDB_V2_TOKEN", '') # InfluxDB V2 Access token, required only for InfluxDB V2
INFLUXDB_V2_ORG = os.getenv("INFLUXDB_V2_ORG", '') # InfluxDB V2 Organization, required only for InfluxDB V2
INFLUXDB_V2_BUCKET = os.getenv("INFLUXDB_V2_BUCKET", 'GarminStats') # InfluxDB V2 Bucket name, required only for InfluxDB V2
...
try:
    if INFLUXDB_VERSION == '2':
        # InfluxDB v2 client initialization
        influxdbclient = InfluxDBClient2(url=INFLUXDB_V2_URL, token=INFLUXDB_V2_TOKEN, org=INFLUXDB_V2_ORG)
        # Test the connection
    ...
    elif INFLUXDB_ENDPOINT_IS_HTTP:
    ...
    # Test the connection with a demo point (skip for v2 as health check already done)
    if INFLUXDB_VERSION != '2':
        demo_point = {
...
def write_points_to_influxdb(points):
    ...
    try:
        if len(points) != 0:
            ...
            for i in range(0, len(points), write_chunk_size):
                ...
                if INFLUXDB_VERSION == '1':
                    ...
                    # Convert points to InfluxDB v2 Point objects
                    influx_points = []
                    for point in chunk:
                    ...
                    # Write points to InfluxDB v2
                    write_api.write(bucket=INFLUXDB_V2_BUCKET, record=influx_points)
                else:  # v3
        ...
        logging.error("Write failed : Unable to write to database! " + str(err))
